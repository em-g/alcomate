...
void settingsMainMenu() {
    ...
    if (buttonPushed == 1) {
        if (settings[chosenOption] == backCommando) {
            ...
            wttd("FLOW COUNTER", 0, 2, "-> " + displayCounter, 1, 0);
            menulvl = 96;
            chosenSetting = settings[chosenOption];
            chosenOption = 0;
        }
...
void showFlowCounter() {
    ...
    if ((rotDet == 2) && (chosenOption < (amountOfDrinks - 1)) && (drinks[chosenOption + 1] == backCommando)) {
        chosenOption++;
        wttd("FLOW COUNTER", 0, 2, "-> " + backCommando, 1, 0);
    } else if ((rotDet == 2) && (chosenOption < (amountOfDrinks - 1))) {
        chosenOption++;
